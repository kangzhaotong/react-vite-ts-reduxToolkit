...
const WebGLFluidSimulation = () => {
  ...
  useEffect(() => {
    ...
    const advectionShader = compileShader(
      ...
      ext.supportLinearFiltering ? undefined : (['MANUAL_FILTERING'] as any[])
    );
    ...
    let ditheringTexture: any = createTextureAsync('LDR_LLL1_0.png');
    ...
    const displayMaterial: any = new Material(
      baseVertexShader,
      displayShaderSource
    );
    ...
    function drawDisplay(target: any) {
      ...
      if (config.SHADING)
        gl.uniform2f(
          displayMaterial.uniforms.texelSize as any,
          1.0 / width,
          1.0 / height
        );
      gl.uniform1i(
        displayMaterial.uniforms.uTexture as any,
        dye.read.attach(0)
      );
      if (config.BLOOM) {
        gl.uniform1i(displayMaterial.uniforms.uBloom as any, bloom.attach(1));
        gl.uniform1i(
          displayMaterial.uniforms.uDithering as any,
          ditheringTexture.attach(2)
        );
        let scale = getTextureScale(ditheringTexture, width, height);
        gl.uniform2f(
          displayMaterial.uniforms.ditherScale as any,
          scale.x,
          scale.y
        );
      }
      if (config.SUNRAYS)
        gl.uniform1i(
          displayMaterial.uniforms.uSunrays as any,
          sunrays.attach(3)
        );
      blit(target);
    }
    ...
    function applySunrays(source: any, mask: any, destination: any) {
      ...
      sunraysProgram.bind();
      gl.uniform1f(
        sunraysProgram.uniforms.weight as any,
