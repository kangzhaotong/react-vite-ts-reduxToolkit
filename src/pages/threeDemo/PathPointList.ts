...
class PathPointList {
    ...
    set(points: any[], cornerRadius = 0.1, cornerSplit = 10, up = null, close = false) {
    ...
    _start(current: any, next: any, up: null) {
        this.count = 0;
    ...
    _corner(current: any, next: any, cornerRadius: any, cornerSplit: any | undefined, up: any) {
    ...
    _sharpCorner(current: Vector3, next: Vector3, up: any, dirType = 0, sharp = false) {
        const lastPoint = this.array[this.count - 1];
        const point = this._getByIndex(this.count);
...
function _getCornerBezierCurve(last: Vector3, current: Vector3, next: Vector3, cornerRadius: number, firstCorner: boolean, out: QuadraticBezierCurve3) {
